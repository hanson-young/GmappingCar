...
void Can_GPIO_Config(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	         //PB8:CAN-RX  重定义功能 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;			   //输入上拉
	GPIO_Init(GPIOB, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;					 //PB9:CAN-TX  重定义功能 
...
//主控向GPS板发送的命令
void USB_LP_CAN1_RX0_IRQHandler(void)   //重定义的用这个中断服务函数
{
	uint8_t i;
	CanRxMsg   RxMessage;
// 	printf("\r\ncan ok");
